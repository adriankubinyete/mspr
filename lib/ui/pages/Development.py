...
class DevelopmentPage(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        tk.Label(frame, text="Development page, please ignore!", font=("Arial", 16)).pack(pady=(10, 0))

        # Lista de configurações e nomes para cada botão
        buttons_config = [
            {"text": "wwwwwwwwwwwwwwwwwwww", "name": "btn_1", "config": {"command": lambda: None}},
            {"text": "Button 2", "name": "btn_2", "config": {"command": lambda: None}},
        ...
        for i, button in enumerate(buttons_config):
            ...
            button_widget = ttk.Button(button_frame, width=10, text=button["text"], **button["config"], takefocus=False)
