...
class BaseModal:
    def __init__(
        self, root, width, height, title, resizeable=False, section_name="debug"
    ):
        # === Menu initialization
        self.root = root
        ...
        self.LOG_NAME = (
            f"mpsr.Modal.{self.section_name}"  # dont forget to name your log.
        )
    ...
    def _widgets(self, parent):
        ...
        raise NotImplementedError(
            "This method needs to be overwritten. You should build your interface here. The first argument is the parent frame on which your interface will be built."
        )

    def open(self):
        """Starts this modal."""
        logger = self._getLogger("open")
        logger.trace("Reading configuration file")
        Config.load()
        if (
            hasattr(self, "modal_root")
            and self.modal
            and tk.Toplevel.winfo_exists(self.modal)
        ):
            return
        ...
        self.modal.grab_set()
        self.modal.protocol(
            "WM_DELETE_WINDOW", self.__on_window_closed_manually
        ...
        if self.SAVE_BUTTON:
            save_button = ttk.Button(
                self.modal, text="Save", command=self.save_and_close, takefocus=False
            )
            save_button.pack(fill="x", padx=10, pady=5)
        else:
            logger.debug("The save button won't be created.")
        ...
        if str(self.WINDOW_HEIGHT).lower().upper() == "AUTO":
            ...
            self.modal.geometry(
                f"{self.WINDOW_WIDTH}x{self.modal.winfo_reqheight()}"
            )  # Ajusta para altura necessária
        else:
    ...
    def _add_tooltip(self, widget, text):
        ...
        label = ttk.Label(
            tooltip, text=text, relief="solid", borderwidth=1, font=("Arial", 9)
        )
    ...
    def _click_assign_show_position_highlight(self, option, duration=5000):
        ...
        overlay.geometry(f"{size}x{size}+{x-offset}+{y-offset}")  # stupid linter # noqa
        ...
        self.active_timers[option] = self.root.after(
            duration, lambda: self._click_assign_remove_highlight(option)
        )
    ...
    def _extract_values_from_entries(self):
        ...
        for key, entry_var in self.entries_to_save.items():
            ...
            elif isinstance(entry_var, tuple) and all(
                isinstance(var, tk.IntVar) for var in entry_var
            ):
                # Se for um Box Assign (x, y, w, h), salva como string "x,y,w,h"
                Config.set(
                    self.section_name,
                    key,
                    ",".join(str(var.get()) for var in entry_var),
                )
            else:
    ...
    def make_ui_click_assign(self, parent, label, info, key, padx=0, pady=0):
        ...
        # logger = self._getLogger("make_ui_click_assign")
        ...
        section = ttk.LabelFrame(
            parent, text=label
        )  # Usa o próprio label como título da seção
        ...
        info_button = ttk.Label(
            row_frame, text=self.SYMBOL_LABEL_INFO, foreground="blue", cursor="hand2"
        )
        ...
        btn_assign.config(
            command=lambda: self._click_assign_capture_position(
                key, spin_x, spin_y, btn_assign
            )
        )
    ...
    def make_ui_entry(self, parent, label, key, info=None, padx=0, pady=0):
        ...
        ttk.Label(row_frame, text=label).pack(
            side="left", padx=(0, SPACING_BETWEEN_ELEMENTS)
        )
        ...
        self.entries_to_save[key] = (
            entry_var  # add to the list of things that will be saved if you press the save button
        )
    ...
    def make_ui_toggleable_entry(self, parent, label, key, info=None, padx=0, pady=0):
        ...
        toggle_var = tk.BooleanVar(
            value=int(Config.get(self.section_name, f"{key}_enabled", "1"))
        )  # Pega do config, padrão ativado
        ...
        self.entries_to_save[f"{key}_enabled"] = (
            toggle_var  # Salva o estado do toggle também
        )
    ...
    def make_ui_checkbox(self, parent, label, key, info=None, padx=0, pady=0):
        ...
        checkbox_var = tk.BooleanVar(
            value=Config.get(self.section_name, key, "0") == "1"
        )
    ...
    def make_ui_box_assign(self, parent, label, key, info=None, padx=0, pady=0):
        ...
        def create_spinbox(parent, text, var):
            ttk.Label(parent, text=f"{text}:", takefocus=False).pack(
                side="left", padx=2
            )
            spinbox = ttk.Spinbox(
                parent, from_=0, to=9999, textvariable=var, width=5, takefocus=False
            )
        ...
        btn_assign = ttk.Button(
            row_frame, text="📷 Assign", command=lambda: assign_box(), takefocus=False
        )
        btn_assign.pack(side="right", padx=5)

        # Botão para mostrar a área
        btn_show = ttk.Button(
            row_frame, text="Show", command=lambda: show_box(), takefocus=False
        )
        ...
        def assign_box():
            ...
            canvas = tk.Canvas(
                overlay, bg="black", highlightthickness=0, takefocus=False
            )
