...
class MultiAccountPage(ttk.Frame):
    ...
    def create_success_frame(self):
        ...
        override_fps_toggleentry = ui.ToggleableEntry(
            test_frame,
            section=self.MULTIACCOUNT_SECTION,
            label="max_fps_override",
            key="max_fps_override",
            info="If enabled, launched accounts will use this as their maximum FPS.",
        )
        override_fps_toggleentry.pack(fill="x", padx=5, pady=5)

        override_server_toggleentry = ui.ToggleableEntry(
            test_frame,
            section=self.MULTIACCOUNT_SECTION,
            label="server_override",
            key="server_override",
            info="If enabled, launched accounts will use this server URL.",
        )
        ...
        self.accounts_treeview.tag_configure(
            "disabled", foreground="#441520", background="#eebbc1"
        )
        ...
        self.treeview_menu.add_command(
            label="Disable/enable account", command=self.context_toggle_account
        )
        self.treeview_menu.add_command(
            label="Launch account", command=self.context_launch_account
        )
        self.treeview_menu.add_command(
            label="Launch main account", command=self.context_launch_main_account
        )
        self.treeview_menu.add_command(
            label="Debuginfo", command=self.context_debug_info
        )
        ...
        self.accounts_treeview.column(
            "Private Server Share URL", width=int(total_width * 0.33)
        )
        self.accounts_treeview.column(
            "Alternate Webhook URL", width=int(total_width * 0.33)
        )
    ...
    def on_edit(self, event):
        ...
        # logger = self.__getLogger("on_edit")
        ...
        def save_edit(event):
            logger = self.__getLogger("save_edit")
            ...
            if column_index == 1:  # Coluna 1: Server URL
                ...
                if (
                    new_value != ""
                ):  # only checks if its a duplicate value if theres content. if its an empty cell, dont check for duplicates
                    for section in Config.get_sections():
                        if (
                            section.startswith(self.RWS_ACCOUNTS_SECTION_PREFIX)
                            and section != account_section
                        ):
                            account_being_checked = section[
                                len(self.RWS_ACCOUNTS_SECTION_PREFIX) :
                            ]
                            if Config.get(section, "server_url") == new_value:
                                ...
                                for iid in self.accounts_treeview.get_children():
                                    item_values = self.accounts_treeview.item(
                                        iid, "values"
                                    )
                                ...
                                if found_in_treeview:
                                    ...
                                    if not result:
                                        logger.debug("User cancelled the swap.")
                                        return
                                    else:
                                        logger.debug("User confirmed the swap.")

                                        # Atualiza o Treeview: encontra o item que tem essa conta
                                        for (
                                            iid
                                        ) in self.accounts_treeview.get_children():
                                            item_values = self.accounts_treeview.item(
                                                iid, "values"
                                            )
                                            if item_values[0] == account_being_checked:
                                        ...
                                        Config.set(
                                            section=section, key="server_url", value=""
                                        )
                                else:
                                    # Conta não existe mais na UI, deletar a seção inválida
                                    logger.warn(
                                        f"Removing orphaned config section: {section}"
                                    )
    ...
    def fetch_accounts(self):
        ...
        logger = self.__getLogger("fetch_accounts")
        logger.debug("Fetching accounts from RAMWS...")
        ...
        def update_treeview(accounts):
            ...
            for account in accounts:
                ...
                account_section = f"{self.RWS_ACCOUNTS_SECTION_PREFIX}{account}"
                logger.trace(f"Fetching config for account: {account}")
                account_status = Config.get(
                    section=account_section, key="enabled", fallback=True
                )  # Assume que está habilitado se não encontrado
                logger.trace(f"Account status: {account_status}")
                ...
                if not account_status:
                    logger.info(f"Account: {account} is disabled.")
                    self.accounts_treeview.item(
                        item_id,
                        values=self.accounts_treeview.item(item_id, "values"),
                        tags=("disabled",),
                    )
        ...
        threading.Thread(
            target=RAMWS.list_accounts,
            args=(update_treeview,),
            daemon=True,
        ).start()
    ...
    def context_toggle_account(self):
        ...
        for item_id in selected:
            ...
            self.toggle_account(item_id)
            self.accounts_treeview.selection_remove(self.accounts_treeview.selection())

    def context_launch_main_account(self):
        selected = self.accounts_treeview.selection()
        for item_id in selected:
            account = self.accounts_treeview.item(item_id, "values")[0]
            private_url = Config.get(
                section=f"{self.RWS_ACCOUNTS_SECTION_PREFIX}{account}",
                key="server_url",
                fallback="",
            )
            self.launch_single_account(
                account_name="masutty", force=True, override_server_url=private_url
            )
    ...
    def context_debug_info(self):
        ...
        account = self.accounts_treeview.item(
            self.accounts_treeview.selection()[0], "values"
        )[0]

        def background():
            logger = self.__getLogger("context_debug")
            try:
                logger.info(f"Account: {account}")
                data = RAMWS.resolve_share_link(
                    resolver_account=account,
                    url="https://www.roblox.com/share?code=37f3ef98bf46aa4897c2768c2dbc8cbc&type=Server",
                )
                logger.trace(data)
            except Exception as e:
                logger.error(f"Error resolving share link...\n{e}")
    ...
    def get_account_config(self, account_name):
        ...
        try:
            ...
            d["is_enabled"] = (
                str(
                    Config.get(
                        section=account_section, key="enabled", fallback=False
                    ).lower()
                )
                == "true"
            )
            d["server_url"] = Config.get(
                section=account_section, key="server_url", fallback=""
            )
            d["webhook_url"] = Config.get(
                section=account_section, key="webhook_url", fallback=""
            )
        except Exception as e:
            logger.error(f"Failed to get account config for {account_name}. Error: {e}")

        return d

    def launch_single_account(
        self, account_name, force=False, override_server_url=None, override_max_fps=None
    ):
        # this is ugly with all the overrides and whatnot, but works
        def background():
            logger = self.__getLogger("launch_single_account")
            try:
                account_data = self.get_account_config(account_name)
                if not account_data["is_enabled"] and not force:
                    logger.warn(f'Account "{account_name}" is disabled.')
                    return

                def on_success(data):
                    logger.info(f'Account "{account_name}" launched successfully.')
                    logger.info(f"Data: {data}")

                    if not SolsBiomeNotifier.is_being_monitored(account_name):
                        logger.trace(f"Setting monitor for {account_name}")
                        SolsBiomeNotifier.on_biome_change(
                            username=account_name,
                            callback=lambda current_biome, previous_biome: print(
                                f"[{account_name}] {previous_biome} -> {current_biome}"
                            ),
                        )
                    else:
                        logger.trace(
                            f"Account {account_name} is already being monitored."
                        )

                def on_fail(data):
                    logger.error(f'Account "{account_name}" failed to launch.')
                    # l.error(f"Data: {data}")

                logger.info(f'Trying to launch account "{account_name}"')
                ...
                if not ServerCodeCache[account_data["server_url"]] and not force:
                    logger.warn("cache miss: " + account_data["server_url"])
                    link_code = RAMWS.resolve_share_link(
                        resolver_account=account_name, url=account_data["server_url"]
                    )
                    if not link_code:
                        logger.error(
                            f'Failed to resolve share link for "{account_name}"'
                        )
                        return
                ...
                if not override_server_url:
                    FPS_OVERRIDE_ENABLED = (
                        str(
                            Config.get(
                                section=self.MULTIACCOUNT_SECTION,
                                key="max_fps_override_enabled",
                                fallback=0,
                            )
                        )
                        == "1"
                    )
                    FPS_OVERRIDE_VALUE = Config.get(
                        section=self.MULTIACCOUNT_SECTION,
                        key="max_fps_override",
                        fallback="",
                    )
                    if FPS_OVERRIDE_ENABLED and FPS_OVERRIDE_VALUE:
                        logger.warn(
                            f'Overriding FPS of account "{account_name}" to {FPS_OVERRIDE_VALUE}'
                        )
                        RAMWS.set_field(
                            account=account_name,
                            field="MaxFPS",
                            value=FPS_OVERRIDE_VALUE,
                        )
                    else:
                        RAMWS.set_field(
                            account=account_name, field="MaxFPS", value="9999"
                        )
                else:
                    RAMWS.set_field(
                        account=account_name, field="MaxFPS", value="9999"
                ...
                if (
                    override_server_url
                ):  # manual override. you REALLY want to launch a different server. ok lets ignore any configuration then
                    logger.warn(
                        f"CRITICAL! Manual override of account {account_name}'s server to {override_server_url}"
                    )
                    ...
                    SERVER_OVERRIDE_ENABLED = (
                        str(
                            Config.get(
                                section=self.MULTIACCOUNT_SECTION,
                                key="server_override_enabled",
                                fallback=0,
                            )
                        )
                        == "1"
                    )
                    SERVER_OVERRIDE_VALUE = Config.get(
                        section=self.MULTIACCOUNT_SECTION,
                        key="server_override",
                        fallback="",
                    )
                    if SERVER_OVERRIDE_ENABLED:
                        logger.warn(
                            f'Overriding server of account "{account_name}" to {SERVER_OVERRIDE_VALUE}'
                        )
                ...
                RAMWS.launch_account(
                    ...
                    callback=lambda data: (
                        on_success(data) if data["success"] else on_fail(data)
                    ),
                )
            except Exception as e:
                logger.error(f"Failed to launch account: {account_name}. Error: {e}")
    ...
    def launch_all_accounts(self):
        ...
        def background():
            logger = self.__getLogger("launch_all_accounts")
            logger.info("Launching all accounts...")
            ...
            try:
                if hasattr(self, "launch_all_button"):
                    ...
                    self.launch_all_button.after(
                        5000,
                        lambda: self.launch_all_button.config(
                            state="normal", text=original_text
                        ),
                    )
            except Exception as e:
                logger.warn(f"Could not modify launch_all_button: {e}")
            ...
            for item in self.accounts_treeview.get_children():
                ...
                try:
                    ...
                    logger.error(f"Error launching account {account}: {e}")

        threading.Thread(target=background, daemon=True).start()

    def on_biome_change(self, account_name, current_biome, previous_biome):
        logger = self.__getLogger("BIOME")
        logger.info(f"[{account_name}] {current_biome} -> {previous_biome}")
