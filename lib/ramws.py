...
class RAMWS:
    ...
    def start_connection_checker(self, callback, interval=10):
        ...
        logger = self.__getLogger("ConnectionChecker")
        ...
        def run():
            ...
            while self.keepalive_running:
                ...
                logger.trace("WebServer is %s", "online" if status else "offline")
    ...
    def list_accounts(self, callback, timeout=30):
        """Lista as contas do usuário a partir do endpoint /GetAccounts."""
        logger = self.__getLogger("list_accounts")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.trace(f"Accounts: {accounts_text}")
                ...
                logger.error(f"Error while listing accounts: {response.status_code}")
                callback([])  # Se a resposta não for 200, retorna uma lista vazia

        except requests.RequestException as e:
            # Caso ocorra uma exceção (como timeout, falha na rede, etc.)
            logger.error(f"Error while listing accounts: {str(e)}")
            callback([])  # Caso ocorra uma exceção, retorna uma lista vazia

    def get_field(self, account, field, timeout=30):
        logger = self.__getLogger("get_field")
        # logger.debug(f"Getting field for account: {account}")
        ...
        try:
            ...
            if not ws_error:
                ...
                logger.error("WS Error while getting field")
                return_data = None
        except requests.RequestException as e:
            logger.error(f"Error while getting field: {str(e)}")

        return return_data

    def set_field(self, account, field, value, timeout=30):
        logger = self.__getLogger("set_field")
        # logger.debug(f"Setting field for account: {account}")
        ...
        try:
            ...
            if not ws_error:
                ...
                logger.error("WS Error while setting field")
                return_data = False
        except requests.RequestException as e:
            logger.error(f"Error while setting field: {str(e)}")

        return return_data

    def remove_field(self, account, field, timeout=30):
        logger = self.__getLogger("remove_field")
        logger.debug(f'Removing field "{field}" for account "{account}"')
        ...
        try:
            ...
            if not ws_error:
                ...
                logger.error("WS Error while removing field")
                return_data = False
        except requests.RequestException as e:
            logger.error(f"Error while removing field: {str(e)}")

        return return_data

    def get_account_info(self, account, timeout=30):
        pass
        # logger = self.__getLogger("get_account_info")
        # logger.debug(f"Getting account info for account: {account}")

    # Dangerous! Needs "Configuration > Developer > Allow GetCookie Method" to be enabled on RAM
    def get_cookie(self, account):
        logger = self.__getLogger("get_cookie")
        # logger.debug(f"Getting cookie for account: {account}")
        ...
        try:
            ...
            if not ws_error:
                ...
                # error
                logger.error("WS Error while getting cookie")
        except requests.RequestException as e:
            logger.error(f"Error while getting cookie: {str(e)}")
    ...
    def get_csrf_token(self, account):
        ...
        logger = self.__getLogger("get_csrf_token")
        # logger.debug(f"Getting CSRF token for account: {account}")
        ...
        try:
            ...
            if not ws_error:
                # success
                logger.trace(
                    f"{account}'s CSRF Token: {response.text[0]}{'*' * (len(response.text) - 2)}{response.text[-1]}"
                )
                ...
                logger.error("WS Error while getting CSRF Token")
                pass
        except requests.RequestException as e:
            logger.error(f"{account} : Failed to get CSRF Token: {str(e)}")
    ...
    def resolve_share_link(self, resolver_account, url, timeout=30):
        ...
        """
        logger = self.__getLogger("shareLinkResolver")
        logger.debug(f"Extracting server code from shared link: {url}")

        # validate it is a valid link:
        if not str(url).lower().startswith(
            "https://www.roblox.com/share?code="
        ) and not str(url).lower().endswith("&type=server"):
        ...
        payload = {
            "linkId": url.split("code=")[1].split("&")[
                0
            ],  # https://www.roblox.com/share?code=00e1ce0a27ab7142bde4aec006b57f01&type=Server
            "linkType": "Server",
        }
        ...
        try:
            ...
            data = response.json()
            logger.trace(f'"{url}": {data}')

            server_data = data.get("privateServerInviteData")
            if server_data and server_data.get("status") == "Valid":
                logger.test("Link is valid")
                return server_data.get("linkCode")
            else:
                logger.test("Link is invalid")
                return None

        except Exception as e:
            logger.error(f"Error while extracting server code: {str(e)}")
            return None
    ...
    def launch_account(
        ...
        logger = self.__getLogger("launch_account")
        logger.debug(f"Launching account: {account}")
        ...
        try:
            ...
            if not ws_error:
                ...
                logger.info(f"Successfully launched account: {account}")
                return_data["success"] = True
            else:
                # Se houver erro, loga e adiciona ao return_data
                logger.error(
                    f"Failed to launch account {account}. Server error: {ws_error}"
                )
                return_data["error"] = ws_error

        except requests.RequestException as e:
            # Captura falhas de conexão, timeout, etc.
            logger.error(f"Error while launching account {account}: {str(e)}")
            return_data["error"] = str(e)
