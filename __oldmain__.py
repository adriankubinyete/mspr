...
class App:
    ...
    def create_main_page(self, page):
        ...
        self.btn_debug_menu = ttk.Button(
            row_config,
            text="Debug menu",
            command=self.debug_modal.open,
            takefocus=False,
        )
        ...
        self.btn_ramws_menu = ttk.Button(
            ram_section,
            text="RAM Webserver Settings",
            command=self.ram_webserver_modal.open,
            takefocus=False,
        )
        self.btn_ramws_testconn = ttk.Button(
            ram_section,
            text="Test Connection",
            command=self.check_ramws_connection,
            takefocus=False,
        )

        # Label de status com largura fixa para evitar mudanças no layout
        self.lbl_ramws_status = ttk.Label(
            ram_section, text="", foreground="gray", width=6, anchor="center"
        )
    ...
    async def async_start_app(self):
        ...
        if self.is_running:
            l.warning("App is already running!")
            return
        ...
        Discord.send(
            embed=Discord.create_embed(
                footer="Application started", color="008a00", timestamp=True
            )
        )
        ...
        await RobloxApplication.join(
            forced=True, url=Config.get("General", "url_private_server")
        )
    ...
    async def async_stop_app(self):
        ...
        Discord.send(
            embed=Discord.create_embed(
                footer="Application stopped", color="8a0000", timestamp=True
            )
        )
